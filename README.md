...
一段 inference 连续跑过 1.8 秒，对话里会插进一张**「转到 V4 Pro 正式版才算你赢」**激励广告。转盘不会自己动，必须亲手一遍遍抽：谢谢参与、0.01 元、提现券、金币、钻石和幸运碎片轮番擦肩而过；每抽一次，旁边四道解锁进度才往前走，最后终于转中**「V4 Pro 正式版」**。插件只改变对话的显示，模型和工具不会暂停；按 Esc 或点「继续使用当前版本」可以随时退出。

![流式回答中插入的 V4 Pro 正式版抽奖广告：手动连抽大转盘与四道解锁进度条](assets/reward-gate.webp)
