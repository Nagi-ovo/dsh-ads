...
一段 inference 连续跑过 1.8 秒，对话里会插进一张**「这一次，你一定要提现」**激励广告。模型和工具都没有暂停：插件只是暂时裁掉当前回答的新内容，并藏起后面新出现的 tool call。8 秒后领取虚拟的 0.01 元，已经产生的内容一次性全部显示。不想等可以直接点「放弃提现」或按 Esc。

![流式回答中插入的推理激励广告：这一次，你一定要提现，还差 0.01 元](assets/reward-gate.webp)
